...
from object_detect.helper.roi_heads import paste_masks_in_image
...
def resize_boxes(boxes, original_size, new_size):
    ...
    if len(boxes) > 0:
        ...
        em = np.empty(0)
        xmin, xmax = torch.tensor(em, dtype=torch.int64), torch.tensor(em, dtype=torch.int64)
        ymin, ymax = torch.tensor(em, dtype=torch.int64), torch.tensor(em, dtype=torch.int64)
