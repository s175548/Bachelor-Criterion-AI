...
def fill_background(img_path,mask_path,name,tif_type):
    img_array=np.array(PIL.Image.open(img_path))
    mask_array=np.array(PIL.Image.open(mask_path))
    data_loader = DataLoader(data_path=r'/work3/s173934/Bachelorprojekt/leather_patches',
    ...
    if tif_type == 'brevetti':
        mask_3d.save(r'/zhome/dd/4/128822/Bachelorprojekt/predictions/tif_brevetti/brevetti_{}_back.png'.format(name))
    else:
        mask_3d.save(r'/zhome/dd/4/128822/Bachelorprojekt/predictions/vda4/vda_{}_back.png'.format(name))
