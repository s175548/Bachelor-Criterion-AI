    ...
    with torch.no_grad():
        ...
        for i in range(len(train_images)):
            ...
            PIL.Image.fromarray(image.astype(np.uint8)).save(save_path + '/{}/{}_{}_{}_img_train.png'.format(model_name, N, i,"tick"+str(lr)),
                                                             format='PNG')
            PIL.Image.fromarray(((pred.squeeze() - 1) * (-255)).astype(np.uint8)).save(os.path.join (save_path , '{}\{}_{}_{}_prediction_train.png'.format(model_name, N, i, "tick" + str(lr)) ), format='PNG')
            PIL.Image.fromarray((target * 255).astype(np.uint8)).save(os.path.join ( save_path , '{}\{}_{}_{}_mask_train.png'.format(model_name, N, i,"tick"+str(lr)) ), format='PNG')
